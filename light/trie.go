...
func NewState(ctx context.Context, head *types.Header, odr OdrBackend) *state.StateDB {
	state, _ := state.New(head.Root, NewStateDatabase(ctx, head, odr), nil, nil)
	return state
}
...
func (db *odrDatabase) OpenStorageTrie_fetch(addrHash, root common.Hash) (state.Trie, error) {
	return &odrTrie{db: db, id: StorageTrieID(db.id, addrHash, root)}, nil
}
...
func (t *odrTrie) TryGetAccountHOTCACHE(key []byte) (*types.StateAccount, error) {
	return nil, nil
}
func (t *odrTrie) TryGetAccountTestisinClean(key []byte) bool {
	return false
}
func (t *odrTrie) AddMess() {
