...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	fmt.Printf("chainConfig:%+v", chainConfig)
	...
	hc := &hotcache.Hotcache{}
	hc.Recorder = hc.NewRWHook()
	bc := &BlockChain{
		hotcache:    hc,
		...
		stateCache: state.NewDatabaseWithConfigClassic(db, &trie.Config{
	...
	head := bc.GetBlockByNumber(cacheConfig.Start_from)
	if _, err := state.New(head.Root(), bc.stateCache, bc.snaps, hc.Recorder); err != nil {
...
func NewBlockChainHot(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, *hotcache.Hotcache, error) {
	if cacheConfig == nil {
		cacheConfig = defaultCacheConfig
	}
	fmt.Printf("cacheConfig:%+v\n\n", cacheConfig)
	...
	txLookupCache, _ := lru.New(txLookupCacheLimit)
	futureBlocks, _ := lru.New(maxFutureBlocks)
	hotcache_init := hotcache.NewHotcache(cacheConfig.HASize, 10)

	bc := &BlockChain{

		hotcache:    hotcache_init,
		chainConfig: chainConfig,
		cacheConfig: cacheConfig,
		db:          db,
		triegc:      prque.New(nil),
		stateCache: state.NewDatabaseWithConfig_hot(db, &trie.Config{
			Cache:     cacheConfig.TrieCleanLimit,
			Journal:   cacheConfig.TrieCleanJournal,
			Preimages: cacheConfig.Preimages,
		}, hotcache_init),
	...
	// Make sure the state associated with the block is available
	head := bc.GetBlockByNumber(cacheConfig.Start_from)
	if _, err := state.New(head.Root(), bc.stateCache, bc.snaps, hotcache_init.Recorder); err != nil {
		// Head state is missing, before the state recovery, find out the
		// disk layer point of snapshot(if it's enabled). Make sure the
		// rewound point is lower than disk layer.
		var diskRoot common.Hash
	...
	return bc, hotcache_init, nil
}
func NewBlockChainHotNni(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, *hotcache.Hotcache, error) {
	if cacheConfig == nil {
		cacheConfig = defaultCacheConfig
	}
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit && time.Now().Second()%10 == 0 {
				fmt.Println("dirty size Out of Limit", float64(nodes)/1024/1024)
			}
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	block, err := it.next()
	bc.hotcache.Recorder.NowBLKNUM = block.NumberU64()
	fmt.Println(bc.hotcache.Recorder.NowBLKNUM)
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		statedb, err = state.New(parent.Root, bc.stateCache, bc.snaps, bc.hotcache.Recorder)

		time.Sleep(1 * time.Second)
		// statedb.Recorder.TxsWrite = make(map[*common.Address]struct{}, 100000)
		// statedb.Recorder.TxsRead = make(map[*common.Address]struct{}, 100000)
		...
		if !bc.cacheConfig.TrieCleanNoPrefetch {
			if followup, err := it.peek(); followup != nil && err == nil {
				throwaway, _ := state.New(parent.Root, bc.stateCache, bc.snaps, nil)
		...
		substart := time.Now()
		//pre_node_invoke_count := bc.hotcache.Node_invoke_count
		receipts, logs, usedGas, err := bc.processor.Process(block, statedb, bc.vmConfig)
		receipts.Len()

		//fmt.Println("当前区块命中率", block.NumberU64(), hit_rate)
		hit_rate_storage := float64(statedb.Hit_storage_num) / float64(statedb.Miss_storage_num+statedb.Hit_storage_num)
		...
		if math.IsNaN(hit_rate_storage) {
			hit_rate_storage = 0
		}
