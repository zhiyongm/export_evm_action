...
func New_fetch(owner common.Hash, root common.Hash, db *Database) (*Trie, error) {
	trie := &Trie{
		owner:    owner,
		db:       db,
		hotcache: db.hotcache,
	...
	if root != (common.Hash{}) && root != emptyRoot {
		rootnode, err := trie.resolveHash_hot_fetch(root[:], nil, false)
		if err != nil {
			return nil, err
		}
		trie.root = rootnode
...
func (t *Trie) tryGet_is_in_clean(origNode node, key []byte, pos int, isinclean bool) (value []byte, newnode node, didResolve bool, err error, isincleans bool) {

	switch n := (origNode).(type) {
	case nil:
		return nil, nil, false, nil, isinclean
	case valueNode:
		return n, n, false, nil, isinclean
	case *shortNode:
		if len(key)-pos < len(n.Key) || !bytes.Equal(n.Key, key[pos:pos+len(n.Key)]) {
			// key not found in trie
			return nil, n, false, nil, isinclean
		}
		value, newnode, didResolve, err, isinclean = t.tryGet_is_in_clean(n.Val, key, pos+len(n.Key), isinclean)
		if err == nil && didResolve {
			n = n.copy()
			n.Val = newnode
		}
		return value, n, didResolve, err, isinclean
	case *fullNode:
		value, newnode, didResolve, err, isinclean = t.tryGet_is_in_clean(n.Children[key[pos]], key, pos+1, isinclean)
		if err == nil && didResolve {
			n = n.copy()
			n.Children[key[pos]] = newnode
		}
		return value, n, didResolve, err, isinclean
	case hashNode:
		child, isinclean_ := t.resolveHash_is_in_clean(n, key[:pos])
		if isinclean_ == false {
			isinclean = false
		}
		value, newnode, _, err, isinclean := t.tryGet_is_in_clean(child, key, pos, isinclean)
		return value, newnode, true, err, isinclean
	default:
		panic(fmt.Sprintf("%T: invalid node: %v", origNode, origNode))
	}
}
func (t *Trie) tryGet_hot(origNode node, key []byte, pos int, ishot bool) (value []byte, newnode node, didResolve bool, err error) {
	switch n := (origNode).(type) {
	case nil:
		return nil, nil, false, nil
	case valueNode:
	...
	case *shortNode:
		...
		value, newnode, didResolve, err = t.tryGet_hot(n.Val, key, pos+len(n.Key), ishot)
		if err == nil && didResolve {
			n = n.copy()
			n.Val = newnode
		}
		return value, n, didResolve, err
	case *fullNode:
		value, newnode, didResolve, err = t.tryGet_hot(n.Children[key[pos]], key, pos+1, ishot)
		if err == nil && didResolve {
			n = n.copy()
			n.Children[key[pos]] = newnode
		}
		return value, n, didResolve, err
	case hashNode:
		child, err := t.resolveHash_hot(n, key[:pos], ishot)
		if err != nil {
			return nil, n, true, err
		}
		value, newnode, _, err := t.tryGet_hot(child, key, pos, ishot)
		return value, newnode, true, err
	default:
		panic(fmt.Sprintf("%T: invalid node: %v", origNode, origNode))
	}
...
func (t *Trie) resolveHash_is_in_clean(n hashNode, prefix []byte) (node, bool) {
	hash := common.BytesToHash(n)
	if node, isinclean := t.db.node_is_in_clean(hash); node != nil {
		return node, isinclean
	}
	return nil, true
}
func (t *Trie) resolveHash_hot(n hashNode, prefix []byte, ishot bool) (node, error) {
	hash := common.BytesToHash(n)
	if node := t.db.node_hot(hash, ishot, 1); node != nil {
		return node, nil
	}
	return nil, &MissingNodeError{Owner: t.owner, NodeHash: hash, Path: prefix}
}
func (t *Trie) resolveHash_hot_fetch(n hashNode, prefix []byte, ishot bool) (node, error) {
	hash := common.BytesToHash(n)
	if node := t.db.node_fetch(hash); node != nil {
		//if node := t.db.node(hash); node != nil {
		return node, nil
	}
	return nil, &MissingNodeError{Owner: t.owner, NodeHash: hash, Path: prefix}
