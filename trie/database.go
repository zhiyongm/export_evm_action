...
type Database struct {
	...
	Node_disk_get_num_fetch_cost uint64
	cleans                       *fastcache.Cache   // GC friendly memory cache of clean node RLPs
	Cleans                       *fastcache.Cache   // GC friendly memory cache of clean node RLPs
	cleans_hot                   *fastcache.Cache   // GC friendly memory cache of clean node RLPs
	hotcache                     *hotcache.Hotcache // GC friendly memory cache of clean node RLPs
	//hot_bloom_data *util.Buffer
	//hot_bloom      filter.Filter
...
func NewDatabaseWithConfig(diskdb ethdb.KeyValueStore, config *Config) *Database {
	...
	db := &Database{
		...
		preimages: preimage,
		hotcache:  &hotcache.Hotcache{Node_disk_get_num: 0},
	}

	return db
}
func NewDatabaseWithConfig_classic(diskdb ethdb.KeyValueStore, config *Config, hotcache_ *hotcache.Hotcache) *Database {
	var cleans *fastcache.Cache
	if config != nil && config.Cache > 0 {
		if config.Journal == "" {
			cleans = fastcache.New(config.Cache * 1024 * 1024)
	...
	// 	for {
	// 		<-t.C
	// 		log.Info("Classic:node_hot函数操作磁盘次数", "num", db.Node_disk_get_num)
	// 	}
	// }()
	return db
}

func NewDatabaseWithConfig_hot(diskdb ethdb.KeyValueStore, config *Config, hotcache *hotcache.Hotcache) *Database {
	var cleans *fastcache.Cache
	if config != nil && config.Cache > 0 {
		if config.Journal == "" {
			cleans = fastcache.New(config.Cache * 1024 * 1024)
	...
	db := &Database{
		...
		cleans:     cleans,
		Cleans:     cleans,
		cleans_hot: fastcache.New(1024 * 1024 * 1024),
		dirties: map[common.Hash]*cachedNode{{}: {
			children: make(map[common.Hash]uint16),
		}},
		preimages: preimage,
...
func (db *Database) node(hash common.Hash) node {
	...
	if db.cleans != nil {
		...
		if enc := db.cleans.Get(nil, hash[:]); enc != nil {
			...
			//if size := len(hash[:]) + len(enc); size > 600 {
	...
	//if db.cleans_hot != nil {
	//	if enc := db.cleans_hot.Get(nil, hash[:]); enc != nil {
	//		//memcacheCleanHitMeter.Mark(1)
	//		//memcacheCleanReadMeter.Mark(int64(len(enc)))
	//		// The returned value from cache is in its own copy,
	//		// safe to use mustDecodeNodeUnsafe for decoding.
...
func (db *Database) node_is_in_clean(hash common.Hash) (node, bool) {
	...
	if db.cleans != nil {
		if enc := db.cleans.Get(nil, hash[:]); enc != nil {
			...
			// The returned value from cache is in its own copy,
			// safe to use mustDecodeNodeUnsafe for decoding.
			return mustDecodeNodeUnsafe(hash[:], enc), true
		}
	}
	//if db.cleans_hot != nil {
	//	if enc := db.cleans_hot.Get(nil, hash[:]); enc != nil {
	...
	return mustDecodeNodeUnsafe(hash[:], enc), false
}

func (db *Database) node_fetch(hash common.Hash) node {
...
func (db *Database) commit_hot(hash common.Hash) error {
...
func (db *Database) node_hot(hash common.Hash, ishot bool, count int) node {
...
func (db *Database) node_hot_fetch(hash common.Hash, ishot bool, count int) node {
...
func (db *Database) dereference(child common.Hash, parent common.Hash) {
	...
	if node.parents == 0 {
		...
		//if db.hotcache.HotCachePool != nil {
		//	db.hotcache.HotCachePool.Del(child.Bytes()) //dirty也跟着删除
		//}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	for db.oldest != oldest {
		...
		//dirty在cap里面不能删除！！！
		//db.hotcache.HotCachePool.Del(db.oldest.Bytes())
		db.oldest = node.flushNext
...
func (c *cleaner) Put(key []byte, rlp []byte) error {
	...
	//if c.db.hotcache != nil {
	//	c.db.hotcache.HotCachePool.Del(hash.Bytes()) //hotcache也跟随删除
	//}
