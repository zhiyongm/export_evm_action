...
func NewStateTrie_fetch(owner common.Hash, root common.Hash, db *Database) (*StateTrie, error) {
	if db == nil {
		panic("trie.NewSecure called without a database")
	}
	trie, err := New_fetch(owner, root, db)
	if err != nil {
		return nil, err
	}
	return &StateTrie{trie: *trie, preimages: db.preimages}, nil
...
func (t *StateTrie) TryGetAccountHOTCACHE(key []byte) (*types.StateAccount, error) {
...
func (t *StateTrie) AddMess() {
	if t.trie.db.cleans != nil {
		...
		for i := 0; i < 8*9; i++ {
			...
			t.trie.db.cleans.Set(b, b)
			t.trie.db.cleans.Get(b, b)
		}
